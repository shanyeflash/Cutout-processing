...
class AIImageMatting(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        universal_mask_btn.click(
            self.generate_mask,
            inputs=[self.universal_input, gr.State(False)],
            outputs=[self.universal_output, self.universal_status]
        )
        ...
        portrait_mask_btn.click(
            self.generate_mask,
            inputs=[self.portrait_input, gr.State(True)],
            outputs=[self.portrait_output, self.portrait_status]
        )
        ...
        return [self.universal_input, self.portrait_input]

    def generate_mask(self, img, is_portrait):
        """生成黑白蒙版"""
        try:
